 ...
 */
export default function canvasEdgeHoversParallel(
  edge,
  source,
  target,
  context,
  ...
  const c = sigma.utils.geom.getCircleIntersection(sX, sY, size, tX, tY, dist);

  // Intersection points of the target node circle:
  const d = sigma.utils.geom.getCircleIntersection(tX, tY, size, sX, sY, dist);
